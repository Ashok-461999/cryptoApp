...
class Settings(BaseSettings):
    ...
    risk_per_trade_inr: float = 100.0  # fixed ₹100 risk per trade (max loss at SL)
    max_deploy_pct: float = 40.0
    usdt_to_inr: float = 83.0
    trading_style: str = "scalp"
    min_rr_for_take: float = 2.0  # 1:2 — ₹100 risk → ₹200 scalp win at T1
    target_profit_inr_min: float = 200.0
    take_profit_inr: float = 200.0  # auto-close WIN when live PnL ≥ ₹200
    scalp_target_inr: float = 200.0  # primary scalp bank target
    leverage_min: int = 25
    leverage_max: int = 40
